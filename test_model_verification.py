...
contrastive_margin=16           # Margin for contrastive loss, not used since contrastive doesn't work
...
# Differentiate between part and full images since in the first case we use only one file, while in the other cases we use the three files provided by the authors of the article
...
if image_type=='part':
    ...
    with torch.no_grad():
        ...
        for images1, images2, labels in tqdm(test_loader, desc="Testing"):
            ...
            if loss_name=='contrastive':    # Doesn't work
    ...
    # Define the levels of difficulty, one file for each
