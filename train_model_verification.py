...
classification_threshold=0.6    # Threshold for binary classification
loss_name='binary-cross-entropy'    # Loss function to use, either 'contrastive' or 'binary-cross-entropy', contrastive doesn't work however
contrastive_margin=16           # Margin for contrastive loss, doesn't work however

# Extract parameters from the model name to be able to import it
...
# Tried smarter custom initialization of the weights because had problems learning, probably this is not the issue
...
model.fc1 = nn.Identity()   # Eliminate the last layer to use the model as a feature extractor 
...
# Create optimizer with different learning rates for the feature extractor and the classifier
...
# Create training and validation subsets using the indices calculated during dataset.__init__()ù
...
for epoch in tqdm(range(num_epochs),leave=False):
    ...
    for images1, images2, labels in tqdm(train_loader, desc=f'Currently running epoch number {epoch+1}', leave=False):  
        ...
        if loss_name=='contrastive':    # This part is not used, contrastive loss doesn't work
